    ...
    describe(":::: FUNCTIONS TESTS ::::",()=>{
        ...
        describe("vote() test",()=>{
            ...
            it("should revert if voter already voted for this proposal",async()=>{
                daoInstance.vote(new BN(0),new BN(1),new BN(0),{from:accounts[0]});
                await expectRevert(daoInstance.vote(new BN(0),new BN(1),new BN(0),{from:accounts[0]}),"already voted");
            })
            it("should update the proposal status function of the vote",async()=>{
                ...
                await daoInstance.vote(new BN(0),new BN(1),new BN(0),{from:accounts[0]});
            ...
            it("should set the final value as the average value of all the 'yes' vote ",async()=>{
                ...
                await daoInstance.vote(new BN(0),new BN(2),new BN(0),{from:accounts[3]});
            ...
            it("should emit a voteSetted event",async()=>{
                expectEvent(await daoInstance.vote(new BN(0),new BN(1),new BN(0),{from:accounts[0]}),"voteSetted",{proposalId:new BN(0),voter:accounts[0],vote:new BN(0),updatedProposalStatus:true});
            })
        })
        describe("closeProposal() test",()=>{
            ...
            it("should set the voting status to 'refused' and value to '0' if vote 'no' win",async()=>{
                await daoInstance.addDaoMember(accounts[0],{from:accounts[0]});
                await daoInstance.addDaoMember(accounts[2],{from:accounts[0]});
                await daoInstance.vote(new BN(0),new BN(1),new BN(1),{from:accounts[0]});
                await daoInstance.vote(new BN(0),new BN(3),new BN(1),{from:accounts[2]});
